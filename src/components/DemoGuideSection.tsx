  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Quy trình DCOTA · Nội bộ</span>
      </div>

      <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 space-y-5 shadow-inner">
        {/* Device selector */}
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold border transition-all ${
                  active ? 'text-white shadow-lg' : 'border-slate-700 text-slate-400 hover:text-slate-200 hover:border-slate-600'
                }`}
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2.5 text-xs font-bold uppercase tracking-wide border-b-2 transition-all ${
                  active ? 'text-slate-100' : 'border-transparent text-slate-500 hover:text-slate-300'
                }`}
        ...
        >
            {section?.note && (
              <div className="flex items-start gap-2.5 p-3 rounded-lg bg-slate-800/60 border border-slate-700">
                <Info size={15} className="text-slate-400 shrink-0 mt-0.5" />
                <p className="text-xs text-slate-300 leading-relaxed">{section.note}</p>
              </div>
            )}

            {section?.requirements && section.requirements.length > 0 && (
              <div>
                <h4 className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-2">Yêu cầu trước khi làm</h4>
            ...
            {section?.stepGroups && section.stepGroups.length > 0 && (
              <div className="space-y-4">
                {section.stepGroups.map((group, gi) => (
                  <div key={gi}>
                    {group.label && (
                      <h4 className="text-sm font-bold text-slate-200 mb-2">{group.label}</h4>
                    )}
                    <ol className="space-y-2">
                      {group.items.map((item, ii) => (
                        <li key={ii} className="flex items-start gap-3 text-sm text-slate-300 leading-relaxed">
                          <span
            ...
            {section?.cases && section.cases.length > 0 && (
              <div className="space-y-3">
                {section.cases.map((c, ci) => (
                  <div key={ci} className="rounded-lg border border-slate-800 bg-slate-950/40 p-4">
                    <div className="flex items-start gap-2 mb-2.5">
                      <HelpCircle size={15} className="text-amber-400 shrink-0 mt-0.5" />
                      <p className="text-sm font-bold text-amber-300 leading-relaxed">{c.situation}</p>
                    </div>
              ...
              <div className="p-4 rounded-xl border border-red-500/30 bg-red-500/10">
                ...
                <ul className="space-y-1.5">
                  {section.doNots.map((item, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-red-300/90 leading-relaxed">

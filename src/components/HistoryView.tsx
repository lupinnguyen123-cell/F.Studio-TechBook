      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm font-medium">Xem lại các ca phân tích lỗi gần đây</p>
          </div>
        </div>
        {history.length > 0 && (
          <button
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {filteredHistory.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-600 transition-all flex flex-col"
                  >
                    <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/30">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-200">{brand?.name}</h4>
                    ...
                    <div className="p-4 space-y-4 flex-1 flex flex-col">
                      <div className="bg-slate-800/20 rounded-lg p-3 border border-slate-800/50">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Mô tả lỗi</span>
                        </div>
                        <p className="text-xs text-slate-300 line-clamp-2 italic">"{item.description}"</p>
                      </div>
                      <div className="bg-slate-800/30 rounded-xl p-3 border border-slate-800/50 flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <Sparkles size={12} style={{ color: brand?.accentColor }} />
                          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Kết quả AI</span>
                        </div>
                        <p className="text-xs text-slate-400 line-clamp-4 leading-relaxed">{item.result}</p>
                      </div>
                      <button
                        onClick={() => onOpenItem(item)}
                        className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-2 mt-auto"
                      >
